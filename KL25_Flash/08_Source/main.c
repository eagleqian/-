...
int main(void)
{
    ...
    uart_init(UART_2, 9600);      //使能串口1，波特率为9600
    ...
    uart_send_string(UART_2, "Hello Uart_1!\r\n");    //串口发送初始化提示


    //5. 使能模块中断
    uart_enable_re_int(UART_2);   //使能串口1接收中断
        ...
        if(framestate==0)
        {
            ...
            if(g_uart_recvBuf[2]=='D'){
                ...
                case 'R':
                    ...
                    //下面函数依次为读出数据存放处、扇区号、扇区内的偏移地址、读取字节数
                ...
                case 'A':
					...
					g_uart_sentBuf[1]  =count+1;               //内容数（内容count+C）
					g_uart_sentBuf[2]  = (uint_8)'C';          //内容起始帧
					//物理读取flash；参数：读取数据存放处；目标地址；读取字节数
                ...
                case 'W':
                    ...
                    offset =g_uart_recvBuf[5]*256+g_uart_recvBuf[6];
                    count = g_uart_recvBuf[7];
                    //写入flash；参数：扇区号，写入扇区的偏移地址；写入字节数，源数据缓冲区首地址
